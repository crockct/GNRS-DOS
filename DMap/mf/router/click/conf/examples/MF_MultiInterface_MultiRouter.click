// Description: Multi interface multi router. Multiple one to one link interfaces, no requirement for topology file.
//   Multiple delivery services supported
...
port_one::MF_Interface($dev1, 0, arp_tbl); //First core interface
port_one -> inQ;

port_two::MF_Interface($dev2, 1, arp_tbl); //Second core interface
port_two -> inQ;

// Third potential interface
// port_three::MF_Interface($dev3, 2, arp_tbl); //Second core interface
// port_three -> inQ;

//start incoming pkt processing
inQ -> inUnq::Unqueue
	-> inCntr_pkt // INFO: This could be moved to one counter per interface
	-> mf_cla::Classifier(
...
// TODO Revisit how to assign things to Thread Scheduler
